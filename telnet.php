...
class Telnet {
	...
	public function login($username, $password) {
		try {
			$this->setRegexPrompt('username>');
			$this->waitPrompt();
			$this->write($username);
			$this->setRegexPrompt('password>');
			$this->waitPrompt();
			$this->write($password);
			$this->setPrompt();
			$this->waitPrompt();
	...
	protected function readTo($prompt) {
		...
		do {
			...
			if (time() > $until_t) {
                // printf("debug: looked for '%s' in %s\n", $prompt, $this->buffer);
			...
			if (!empty($prompt) && preg_match("/{$prompt}$/", $this->buffer)) {
                // printf("debug: was lookin for (%s) in the %s\n", $prompt, $this->buffer);
				return self::TELNET_OK;
			}
	...
	protected function write($buffer, $add_newline = TRUE) {
        ...
        // echo "debug: buffer cleared, adding new line...\n";
		if ($add_newline == TRUE) {
			$buffer .= "\r\n";
		}
        // echo "debug: adding buffer to global... \n";
		$this->global_buffer .= $buffer;
        $bytes = fwrite($this->socket, $buffer);
        // printf("debug: fwrite = %d bytes\n",$bytes);
		if (!$bytes < 0) {
			throw new Exception("Error writing to socket");
		}
        // echo "debug: global = " . $this->global_buffer . "\n";
